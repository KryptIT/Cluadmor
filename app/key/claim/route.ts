...
h1{font-size:20px;margin:0 0 8px}.muted{color:#8ea3a8}.key{margin:20px 0;padding:14px;border-radius:9px;background:#05090b;border:1px solid #18343a;font:13px ui-monospace,monospace;word-break:break-all}
...
export async function GET(req: Request) {
  ...
  if (!session) {
    return page(
      "Claudmor",
      "<h1>Key link expired</h1><p class=\"muted\">This key link was already used or has expired. Generate a new one from the script.</p>",
      410
    );
  }
  ...
  const services = await sql`
    SELECT
      ...
      COALESCE(sks.key_duration_minutes, 1440) AS key_duration_minutes
    FROM services s
    LEFT JOIN service_key_settings sks ON sks.service_id = s.id
    WHERE s.id = ${session.service_id}
    LIMIT 1
  `;
  ...
  const minutes = Math.max(1, Number(service.key_duration_minutes) || 1440);
  const key = opaque("CLM", 24);
  const expiresAt = new Date(Date.now() + minutes * 60_000);

  await sql`
    INSERT INTO license_keys(service_id, key_hash, expires_at)
    VALUES (
      ${session.service_id},
      ${digest(key)},
  ...
  const safeExpiry = escapeHtml(expiresAt.toLocaleString("en-US", { timeZone: "UTC" }) + " UTC");

  return page(
    safeName + " key",
    `<h1>${safeName}</h1>
<p class="muted">Your key is ready. Copy it and return to Roblox.</p>
...
<p class="muted" style="margin-top:14px">Expires: ${safeExpiry}</p>`
  );
}
