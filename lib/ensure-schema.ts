...
export function ensureWorkspaceSchema() {
  if (!ready) {
    ready = (async () => {
      ...
      await sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_tickets_session_jti ON bootstrap_tickets(session_jti)`;
      ...
      await sql`CREATE INDEX IF NOT EXISTS idx_runtime_sessions_key ON runtime_sessions(key_id)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_runtime_sessions_expiry ON runtime_sessions(expires_at)`;
      ...
      await sql`
        ALTER TABLE service_loaders
          ADD COLUMN IF NOT EXISTS bootstrap_ciphertext text
      `;
      ...
      await sql`CREATE INDEX IF NOT EXISTS idx_telemetry_ip_created ON telemetry_events(ip_hash, created_at DESC)`;
    })().catch(error => {
      ready = null;
      throw error;
    });
